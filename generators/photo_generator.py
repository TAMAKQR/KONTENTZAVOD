...
class PhotoGenerator:
    ...
    def _create_photo_prompt(
        ...
        - Общий стиль видео
        - ✅ АВТОМАТИЧЕСКОЕ определение: сохранить ли оригинальный текст
        """
        
        scene_prompt = scene.get("prompt", "")
        atmosphere = scene.get("atmosphere", "")
        duration = scene.get("duration", 5)
        preserve_text = scene.get("preserve_text", None)  # Опциональный флаг
        
        # Если есть референс - указываю его в промте
        reference_instruction = ""
        if reference_image_url:
            reference_instruction = "Сохрани стиль и визуальные элементы из референса. "
        
        # Информация о позиции для плавности переходов
        position_context = f"Сцена {scene_index + 1} из {total_scenes}"
        if scene_index > 0:
            position_context += " - продолжение предыдущей, плавный переход"
        if scene_index < total_scenes - 1:
            position_context += " - подготовка к следующей сцене"
        
        # 🎯 УМНОЕ ОПРЕДЕЛЕНИЕ: нужен ли текст?
        # Ищем признаки названий брендов/товаров в промте
        brand_keywords = [
            "baccarat", "rouge", "parfum", "extrait", "флакон",  # парфюм
            "логотип", "бренд", "название", "надпись", "шрифт",  # текст на товаре
            "этикет", "label", "brand name", "logo", "text",
            "шампанского", "вино", "бутылк", "флакон",  # алкоголь/ликёры
            "книг", "обложк", "название книги", "автор", "издател"  # книги
        ]
        
        # Проверяем наличие брендовых/текстовых ключевых слов
        scene_lower = scene_prompt.lower()
        has_brand_keywords = any(keyword in scene_lower for keyword in brand_keywords)
        
        # Определяем нужно ли сохранять текст
        should_preserve_text = preserve_text if preserve_text is not None else has_brand_keywords
        
        # Формируем инструкцию о тексте
        if should_preserve_text:
            text_instruction = "⚠️ ВАЖНО: Сохрани видимый оригинальный текст, логотип, надпись на товаре. Это ключевая часть изображения.\n"
        else:
            text_instruction = "⚠️ ВАЖНО: Без лишнего текста, надписей и логотипов. Только визуальный контент.\n"
        
        # ✅ Формирую финальный промт с УМНЫМ определением текста
        extended_prompt = (
        ...
        logger.info(f"   📝 Инструкция по тексту: {'СОХРАНИ текст' if should_preserve_text else 'БЕЗ текста'}")
